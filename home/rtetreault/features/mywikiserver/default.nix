{
  ...
  systemd.user.services.mywikiserver = {
    Unit = {
      ...
      AssertDirectoryNotEmpty="${config.cloud.path}/mytiddlywiki";
    };
    ...
    Service = {
      ...
      ExecStart = "${pkgs.nodePackages_latest.tiddlywiki}/bin/tiddlywiki ${config.cloud.path}/mytiddlywiki --listen host=0.0.0.0 port=8088 root-tiddler=$:/core/save/lazy-images";
    };
